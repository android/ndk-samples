...
typedef enum {
   ...
   * C_t =& clip(f_t \cdot C_{t-1} + i_t \cdot g(W_{xc}x_t+W_{hc}h_{t-1}+b_c),\ t_{cell})& \\
   * o_t =& \sigma(W_{xo}x_t+W_{ho}h_{t-1}+W_{co}C_t+b_o)& \\
   *      & clip(W_{proj}(o_t \cdot g(C_t))+b_{proj},\ t_{proj}) & if\ there\ is\ a\ projection; \\
   * h_t =& & \\
   *      & o_t \cdot g(C_t) & otherwise. \\
   * \f}
   * Where:
   ...
   * * \f$g\f$ is the fused activation function, usually \f$tahn\f$,
   ...
   * * \f$b_f\f$ is the cell bias,
   * * \f$W_{xo}\f$ is the input-to-output weight matrix,
   * * \f$W_{ho}\f$ is the recurrent-to-output weight matrix,
   * * \f$W_{co}\f$ is the cell-to-output weight matrix,
   * * \f$b_o\f$ is the output gate bias,
   * * \f$g\f$ is the activation function,
   ...
   * * If the input gate has a value, i.e., if coupling of input and forget
   *   gate (CIFG) is used, “input_to_forget_weights”,
   *   “recurrent_to_input_weights”, “cell_to_input_weights”, and
   *   “input_gate_bias” should have no value.
   * * If no peephole connections, “cell_to_input_weights”,
   *   “cell_to_forget_weights”, and “cell_to_output_weights” should have no
   *   value.
   * * If no projection layer, “projection_weights” and “projection_bias”
   *   should have no value.
   * * If no projection bias, “projection_bias” should have no value.
   *
   * References:
   *
   ...
   *      A 2-D tensor of type T, of shape [num_units, output_size].
   * * 9: The cell-to-input weights.
   *      A 1-D tensor of type T, of shape [num_units].
   * * 10:The cell-to-forget weights.
   *      A 1-D tensor of type T, of shape [num_units].
   * * 11:The cell-to-output weights.
   *      A 1-D tensor of type T, of shape [num_units].
   * * 12:The input gate bias.
   *      A 1-D tensor of type T, of shape [num_units].
   * * 13:The forget gate bias.
   *      A 1-D tensor of type T, of shape [num_units].
   * * 14:The cell bias.
   *      A 1-D tensor of type T, of shape [num_units].
   * * 15:The output gate bias.
   *      A 1-D tensor of type T, of shape [num_units].
   * * 16:The projection weights.
   *      A 2-D tensor of type T, of shape [output_size, num_units].
   * * 17:The projection bias.
   *      A 1-D tensor of type T, of shape [output_size].
   * * 18:The output state (in).
   *      A 2-D tensor of type T, of shape [batch_size, output_size].
   * * 19:The cell state (in).
   *      A 2-D tensor of type T, of shape [batch_size, num_units].
   * * 20:The fused activation function.
   *      An optional value indicating the activation function:
   *      0: None;
   *      1: Relu;
   *      3: Relu6;
   *      4: Tanh;
   *      6: Sigmoid.
   * * 21:The clipping threshold for the cell state, such that values are bound
   *      within [-cell_clip, cell_clip]. If set to 0.0 then clipping is
   *      disabled.
   * * 22:The clipping threshold for the output from the projection layer, such
   *      that values are bound within [-proj_clip, proj_clip]. If set to 0.0
   *      then clipping is disabled.
   *
   * Outputs:
   * * 0: The scratch buffer.
   *      A 3-D tensor of type T, of shape [batch_size, num_cell * 4] with
   *      CIFG, or [batch_size, num_cell * 3] without CIFG.
   * * 1: The output state (out).
   *      A 2-D tensor of type T, of shape [batch_size, output_size].
   * * 2: The cell state (out).
   *      A 2-D tensor of type T, of shape [batch_size, num_units].
   * * 3: The output.
...
typedef enum {
  /** SAME padding.
   * For total number padding of n:
   *     padding_to_beginning =  n / 2
   *     padding_to_end       = (n+1)/2.
   * i.e., for even number of padding, padding to both ends are
   * the same; for odd number of padding, padding to the end is bigger
   * than the padding to the beginning by 1.
   * Padding size:
   *    n = filter_size - (input_size % filter_size)
   *    when stride = filter_size
   */
      ANEURALNETWORKS_PADDING_SAME = 1,

  /** VALID padding.
   * No padding. When input size is not evenly divisible by filter size,
   * The input at the end that could not fill the whole filter tile will be
   * simply ignored.
   */
 ...
 */
extern int (*ANeuralNetworksMemory_createFromFd)(size_t size, int protect, int fd, size_t offset,
 ...
 */
extern void (*ANeuralNetworksModel_free)(ANeuralNetworksModel* model);
 ...
 * @return ANEURALNETWORKS_NO_ERROR if successful.
 */
extern int (*ANeuralNetworksModel_finish)(ANeuralNetworksModel* model);
 ...
 *
 */
extern int (*ANeuralNetworksModel_identifyInputsAndOutputs)(ANeuralNetworksModel* model, uint32_t inputCount,
                                                            const uint32_t* inputs, uint32_t outputCount,
                                                            const uint32_t* outputs);
 ...
 */
extern int (*ANeuralNetworksCompilation_create)(ANeuralNetworksModel* model,
 ...
 *                    results in no operation.
 */
extern void (*ANeuralNetworksCompilation_free)(ANeuralNetworksCompilation* compilation);
 ...
 * @return ANEURALNETWORKS_NO_ERROR if successful.
 */
extern int (*ANeuralNetworksCompilation_setPreference)(ANeuralNetworksCompilation* compilation,
                                                       int32_t preference);
 ...
 * @return ANEURALNETWORKS_NO_ERROR if successful.
 */
extern int (*ANeuralNetworksCompilation_finish)(ANeuralNetworksCompilation* compilation);
 ...
 */
extern int (*ANeuralNetworksExecution_create)(ANeuralNetworksCompilation* compilation,
 ...
 *                  results in no operation.
 */
extern void (*ANeuralNetworksExecution_free)(ANeuralNetworksExecution* execution);
 ...
 */
extern int (*ANeuralNetworksExecution_setInput)(ANeuralNetworksExecution* execution, int32_t index,
 ...
 *         name is not recognized or the buffer is too small for the input.
 */
extern int (*ANeuralNetworksExecution_setInputFromMemory)(ANeuralNetworksExecution* execution, int32_t index,
                                                          const ANeuralNetworksOperandType* type,
 ...
 */
extern int (*ANeuralNetworksExecution_setOutput)(ANeuralNetworksExecution* execution, int32_t index,
 ...
 *         name is not recognized or the buffer is too small for the output.
 */
extern int (*ANeuralNetworksExecution_setOutputFromMemory)(ANeuralNetworksExecution* execution, int32_t index,
                                                           const ANeuralNetworksOperandType* type,
                                                           const ANeuralNetworksMemory* memory, size_t offset,
                                                           size_t length);
 ...
 * @return ANEURALNETWORKS_NO_ERROR if successful.
 */
extern int (*ANeuralNetworksExecution_startCompute)(ANeuralNetworksExecution* execution,
...
/**
 * Load library and retrieve entry pointers
 * @return
 */
