...
static GL_WIDECOLOR_MODE_CFG glWideColorCfg[] = {
    {EGL_GL_COLORSPACE_DISPLAY_P3_PASSTHROUGH_EXT, 8, 8, 8, 8},
    {EGL_GL_COLORSPACE_DISPLAY_P3_PASSTHROUGH_EXT, 10, 10, 10, 2},
    {EGL_GL_COLORSPACE_DISPLAY_P3_PASSTHROUGH_EXT, 16, 16, 16, 16},
    {EGL_GL_COLORSPACE_DISPLAY_P3_EXT, 8, 8, 8, 8},
    {EGL_GL_COLORSPACE_DISPLAY_P3_EXT, 10, 10, 10, 2},
    {EGL_GL_COLORSPACE_DISPLAY_P3_EXT, 16, 16, 16, 16},
    {EGL_GL_COLORSPACE_SRGB_KHR, 8, 8, 8, 8},
};

static bool CheckRequiredEGLExt(EGLDisplay disp,
...
bool ImageViewEngine::CreateWideColorCtx(WIDECOLOR_MODE mode) {
  ...
  std::vector<EGLint> attributes{
      ...
      EGL_OPENGL_ES3_BIT,      EGL_BLUE_SIZE,           glWideColorCfg[mode].b_,
      EGL_GREEN_SIZE,          glWideColorCfg[mode].g_, EGL_RED_SIZE,
      glWideColorCfg[mode].r_, EGL_ALPHA_SIZE,          glWideColorCfg[mode].a_,
  };
  ...
  EGLint cfgCount = 1;
  EGLConfig config;
  status = eglChooseConfig(display_, attributes.data(), &config, cfgCount,
                           &cfgCount);
  ...
  attributes.push_back(EGL_NONE);
  eglContext_ =
      eglCreateContext(display_, config, EGL_NO_CONTEXT, attributes.data());
  ...
  attributes.push_back(EGL_NONE);
  surface_ =
      eglCreateWindowSurface(display_, config, app_->window, attributes.data());
  ...
  status = eglMakeCurrent(display_, surface_, surface_, eglContext_);
...
bool ImageViewEngine::CreateWideColorCtx(void) {
  ...
  std::vector<std::string> passthruExt{
      "EGL_KHR_gl_colorspace", "GL_EXT_gl_colorspace_display_p3_passthrough"};
   ...
   */
  std::vector<std::string> p3Exts{"EGL_KHR_gl_colorspace",
  ...
  if (CheckRequiredEGLExt(display_, passthruExt)) {
    ...
    LOGW(
        "====Warning: Display P3 is not supported,"
  ...
  for (auto mode : modes) {
    if (CreateWideColorCtx(mode)) return true;
  }
  return false;
}
