...
PNGHeader::PNGHeader(std::string& name, uint8_t* buf, uint64_t len)
  ...
  while (offset_ < length_) {
    ...
    /*
     * Read the len word ( 4 bytes ), it is only for data field - no self, not
     * for type not for CRC
     */
    ...
    switch (type.value) {
      ...
      case PNG_CHUNCK('c', 'H', 'R', 'M'): {
        littleEndianUint32 val;
        ASSERT(len.value == (2 * 4 * 4), "cHRM Chunk length is not 16(%d)",
               len.value);
      ...
      case PNG_CHUNCK('i', 'C', 'C', 'P'):
        LOGI("====iCCP: %s, compression Method %d", &buf_[offset_],
             buf_[offset_ +
        ...
        break;
      default:
        LOGV("====Unprocessed CHUNK %c%c%c%c", type.bytes[3], type.bytes[2],
             type.bytes[1], type.bytes[0]);
        [[fallthrough]];
...
float PNGHeader::GetGamma() const { return gamma_; }
...
bool PNGHeader::HasNPM(void) const { return hasChrm_; }
