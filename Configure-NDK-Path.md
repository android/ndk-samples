...
Formal releases could be installed with all installation channels, as it is now for Android R time, but the developer preview releases are only installable through Android SDK Manager (UI or command line tool) that has a **canary** channel.
...
From here, all <code>[android.ndkVersion](#ndkversion)</code> path searching rules apply, check each AGP version in this documentation about the designed behaviors.
